...
let glowColor;
let glowWindow;
let buildingColorDay, buildingColorNight; // building colors
...
function setup() {
  ...
  // building colors
  buildingColorDay = color(212, 202, 195);  // day
  buildingColorNight = color(23, 17, 14); // night
}

function draw() {
  // Calculate how much to blend based on sun (mouse) Y position
  let blendAmount = map(mouseY, 0, height, 0, 1, true); // constrained between 0 and 1

  // Create a blended gradient end color that transitions from blue to orange
  let currentBG2 = lerpColor(bg1, bg2, blendAmount);

  // Gradient background when sun sets
  gradientBG(0, 0, width, height, bg1, currentBG2);
  
  // Calculate building color based on sun position
currentBuildingColor = lerpColor(buildingColorDay, buildingColorNight, blendAmount);
  ...
  noStroke();
  fill(glowColor);
  ellipse(width/2, mouseY, 140, 140);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    ellipse(width/2, mouseY, 140 + i * 10, 140 + i * 10);
  }
  
  // Buildings
  stroke(0);
  fill(currentBuildingColor);
  ...
  // Gradient Background
  function gradientBG(x, y, w, h, bg1, bg2) {
    noFill();
